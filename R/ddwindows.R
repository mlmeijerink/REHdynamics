...
#' 
#' @param mintime minimum window width.
...
#' @inheritParams dyneval
#' 
#' @return List with elements "windows" and "options".
#' @export
ddwindows <- function(edgelist, tie_effects, mintime, covar, actors = NULL, stop.rule = TRUE, K = 3, directed = TRUE, ncores = 2) {
    # Event times
	times <- edgelist$time
	tau <- max(times)
